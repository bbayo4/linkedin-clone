...
const tags = ['devops', 'productmanagement', 'react', 'frontend', 'typescript', 'ui-dev', 'ux-design'];
const groups = ['ReactJS', 'Angular', 'Java', 'Islam'];
    ...
    <Container>
      <Panel>
        ...
        <div className={`tags-list${showTags ? '' : ' hidden'}`}>
          {tags.map(item => (
            <span key={item} className="tag" onClick={() => openPage(`feed/hashtag/${item}/`)} data-tip={`Search #${item} hashtag`}>
              <HashtagIcon />
              {item}
            </span>
          ))}
